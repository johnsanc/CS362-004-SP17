...
int checkAdventurerCard(int p, struct gameState *post) {
  ...
  // hand.
  handBefore = post->handCount[p];
  deckBefore = post->deckCount[p];
  discardBefore = post->discardCount[p];

  // Run the function
  checkCard = playAdventurer(post);
  drawCorrect = post->hand[p][post->handCount[p] - 1];
  drawIncorrect = post->hand[p][post->handCount[p]];
