...
int checkCouncil_Room(struct gameState *state, int p) {
  struct gameState correct;
  memcpy(&correct, state, sizeof(struct gameState));

  int result;

  result = playCouncil_Room(state, 0);


  if (correct.deckCount[p] > 0) {
    int counter = 0;
    // Simulate drawing 4 cards
    while (correct.deckCount[p] > 0 && counter < 4) {
      correct.deckCount[p]--;
      correct.handCount[p]++;
      counter++;
    }
  ...
  // Simulate updating number of buys
  correct.numBuys++;
  ...
  if (correct.numBuys > state->numBuys) {
    // printf("TEST2 FAILED\n");
    return 0;
  }
...
int main() {
  ...
  for (n = 0; n < 2000; n++) {
    ...
    result = checkCouncil_Room(&G, 0);
  }

  if (result == 0) {
    printf("COUNCIL ROOM FAILED\n");
  } else {
